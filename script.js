...
function AllQuestionsConstructor(origArray) {
	...
	this.displayedQuestionNumber = 1; // new

	this.increaseIndex = function() {this.currentIndex++;};
	this.decreaseIndex = function() {this.currentIndex--;};
//	this.currentQuestion = function() {return this.originalObjects[this.currentIndex].question;}; // uses number from questionaire
//	this.currentQuestion = function() {return this.currentIndex + 1 + ". " + this.originalObjects[this.currentIndex].question;}; // uses currentIndex as nr
	this.currentQuestion = function() {return this.displayedQuestionNumber + ". " + this.originalObjects[this.currentIndex].question;}; // uses consecutive nr
	this.choicesNo = function() {return this.originalObjects[this.currentIndex].choices.length;};
	this.allChoices = function() {return this.originalObjects[this.currentIndex].choices;}; // returns an array
	this.correctChoices = function() {return this.originalObjects[this.currentIndex].correctAnswer;}; // returns an array
	
//	this.setInitialIndex = function() {this.currentIndex = prompt("introduceti nr intrebarii ", "0"); initialLoad(this); };
	
	this.setInitialIndex = function() {this.currentIndex = document.getElementById("initialOffset").value - 1; };
	this.setRandomIndex = function() {this.currentIndex = Math.floor((Math.random() * this.allQuestionsNo) + 1);}; // new
	this.increaseDisplayedQuestionNumber = function() {this.displayedQuestionNumber += 1;}; // new
	this.decreaseDisplayedQuestionNumber = function() {this.displayedQuestionNumber -= 1;}; // new
}
...
var scoreObj = {
	...
	decrease: function() {this.score--;},
	get: function() {return this.score;}
};

var randomIndexesObj = {
	randomIndexes: [],
	getCurrent: function() {return this.randomIndexes[allQuestionsObj.displayedQuestionNumber];},
	getPrevious: function() {return this.randomIndexes[allQuestionsObj.displayedQuestionNumber - 1];},
	getLast: function() {return this.randomIndexes[this.randomIndexes.length - 1];},
	getNext: function() {return this.randomIndexes[allQuestionsObj.displayedQuestionNumber + 1];},
	getLength: function() {return this.randomIndexes.length;},
	recordIndex: function() {this.randomIndexes.push(allQuestionsObj.currentIndex);},
	removeLastIndex: function() {if(this.randomIndexes.length > 0) this.randomIndexes.pop();}
};
...
// display initial pannel
createInitialPannelHTML(allQuestionsObj);

// at click on START button execute runQuiz() function
document.getElementById("startBtn").addEventListener("click", runQuiz, true);

// read data from initial pannel
// do not validate data because there are default values
// delete initial pannel
// run the corresponding random or consecutive main driver

function runQuiz(){
	
if(isRandomMode()) {
	
	allQuestionsObj.setRandomIndex(); // set currentIndex
	randomIndexesObj.recordIndex();
	removeInitialPannelHTML();
	initialLoad.call(this, allQuestionsObj); // display first question
	...
	var goNextRandomCall = function() {goNextRandom(allQuestionsObj, scoreObj, userAnswersRecorderObj, randomIndexesObj);};
  document.getElementById("nextBtn").addEventListener("click", goNextRandomCall);

	var goBackRandomCall = function() {goBackRandom(allQuestionsObj, scoreObj, userAnswersRecorderObj, randomIndexesObj);};
  document.getElementById("backBtn").addEventListener("click", goBackRandomCall);
	
	
	
}else{ // is consecutive mode

	allQuestionsObj.setInitialIndex(); // set currentIndex
	removeInitialPannelHTML();
	initialLoad.call(this, allQuestionsObj); // display first question
...
//	alert(questObj.correctChoices());
    if (questObj.correctChoices()[1]== 9) {
        var correctAnswerContainerId = "label" + questObj.correctChoices()[0]; // answer-container  label
        document.getElementById(correctAnswerContainerId).style.backgroundColor = "yellow";
    }
    else{
        var correctAnswerContainerId = "label" + questObj.correctChoices()[0]; // answer-container  label
        var correctAnswerContainerId1 = "label" + questObj.correctChoices()[1];
        document.getElementById(correctAnswerContainerId).style.backgroundColor = "yellow";
...
function goNext(questObj, scObj, ansObj) {
	...
	if(questObj.currentIndex < questObj.allQuestionsNo) {
		...
		if(! skipQuestion) {
			...
			computeScore(questObj, scObj);  // medicine
		}
  ...
  questObj.increaseIndex();
	questObj.increaseDisplayedQuestionNumber();

	if(skipQuestion) {
		questObj.decreaseIndex();
		questObj.decreaseDisplayedQuestionNumber();
	}
...
// 	displayScore(questObj, ansObj);
	displayScore(questObj, scObj);  // old version computing and displaying at each click
...
// var goNextCall = function() {goNext(allQuestionsObj, scoreObj, userAnswersRecorderObj);};
// document.getElementById("nextBtn").addEventListener("click", goNextCall); // uncomment !!!



function goNextRandom(questObj, scObj, ansObj, rndIndexesObj) {
...
//	alert("length = " + rndIndexesObj.getLength());

	if(questObj.displayedQuestionNumber < rndIndexesObj.getLength() ){ // comming from back 
		var nextRandomIndex = rndIndexesObj.getCurrent(); // one less 
		setCurrentIndex(questObj, nextRandomIndex);
		questObj.increaseDisplayedQuestionNumber();
	}else{ // normal run
		questObj.setRandomIndex(); // set currentIndex
		rndIndexesObj.recordIndex();
		questObj.increaseDisplayedQuestionNumber();
	}

	if(skipQuestion) {
		rndIndexesObj.removeLastIndex(); // removes the last random index from array
		lastRandomIndex = rndIndexesObj.getLast();
		setCurrentIndex(questObj, lastRandomIndex);
		
//		questObj.decreaseIndex(); // replace for random
		questObj.decreaseDisplayedQuestionNumber();
	}
	...
	if(questObj.currentIndex < questObj.allQuestionsNo) {
		...
		replaceQuestion(questObj);
		createRadiosHTML(questObj);
		if(questObj.displayedQuestionNumber <= Object.keys(ansObj).length) {  // new
			rememberUserAnswers(questObj, ansObj);
		}
...
function setCurrentIndex(questObj, index){
	questObj.currentIndex = index;
}
...
function goBackRandom(questObj, scObj, ansObj, rndIndexesObj) {

	if(questObj.displayedQuestionNumber >= 1) {
...
//		computeScore(questObj, scObj);  // change function
	...
	if(questObj.displayedQuestionNumber > 1) {
		questObj.decreaseDisplayedQuestionNumber();
		questObj.currentIndex = rndIndexesObj.getPrevious();
	}
...
function createInitialPannelHTML(questObj) {
	var x, y, z, i, j, cdivrnd, cdivconsecutive;

	var initialPannel = document.getElementById("initialPannel");
	
		// create a container div for random radio button
		cdivrnd = document.createElement("DIV");
		cdivrnd.setAttribute("id", "rndRadioContainerId")
		
		// create a radio button for random mode, set attributes and then append it to rndRadioContainer div
		x = document.createElement("INPUT");
		x.setAttribute("type", "radio");
		x.setAttribute("name", "initialPannelRadio");
		x.setAttribute("id", "rndRadioId");
		x.setAttribute("checked", "true");
		cdivrnd.appendChild(x);
		
		// create label for the random radio button
		y = document.createElement("LABEL");
		y.setAttribute("for", "rndRadioId");
		y.setAttribute("id", "rndRadioLabelId");
		// append text to random label
		z = document.createTextNode("Intrebari aleatoare");
		y.appendChild(z);
		// append label to random div
		cdivrnd.appendChild(y);
		// append random RadioContainer div to initialPannel div
		initialPannel.appendChild(cdivrnd);
		
		// create a container div for consecutive radio button
		cdivconsecutive = document.createElement("DIV");
		cdivconsecutive.setAttribute("id", "consecutiveRadioContainerId")
		
		// create a radio button for random mode, set attributes and then append it to consecutiveRadioContainer div
		x = document.createElement("INPUT");
		x.setAttribute("type", "radio");
		x.setAttribute("name", "initialPannelRadio");
		x.setAttribute("id", "consecutiveRadioId");
		cdivconsecutive.appendChild(x);
		
		// create label for the consecutive radio button
		y = document.createElement("LABEL");
		y.setAttribute("for", "consecutiveRadioId");
		y.setAttribute("id", "consecutiveRadioLabelId");
		// append text to random label
		z = document.createTextNode(`Intrebari consecutive incepand de la numarul: `);
		y.appendChild(z);
		// append label to consecutive div
		cdivconsecutive.appendChild(y);
		
		// create a text button for initial offset, set attributes and then append it to consecutiveRadioContainer div
		j = document.createElement("INPUT");
		j.setAttribute("type", "number");
		j.setAttribute("name", "initialOffset");
		j.setAttribute("id", "initialOffset");
		j.setAttribute("value", "1");
		j.setAttribute("min", "1");
		j.setAttribute("max", questObj.allQuestionsNo);
		cdivconsecutive.appendChild(j);
		
		z = document.createTextNode(` [max ${questObj.allQuestionsNo}]`);
		cdivconsecutive.appendChild(z);
				
		// append consecutiveRadioContainer div to initialPannel div
		initialPannel.appendChild(cdivconsecutive);
		
		// create start button
		x = document.createElement("BUTTON");
		x.setAttribute("type", "button");
		x.setAttribute("id", "startBtn"); 
		// Create text node and append it to the button
		y = document.createTextNode("START");
		x.appendChild(y);
		// Append button to the initial pannel div
		initialPannel.appendChild(x);
}

// for automatical check of the consecutive radio button when text input if the initial question number gets focused
document.getElementById("initialOffset").addEventListener("focus", checkRadioButton, true);

function checkRadioButton() {  // "consecutiveRadioId"
	document.getElementById("consecutiveRadioId").checked = true;
}
...
function removeInitialPannelHTML() {

	var initPannel;
	
	initPannel = document.getElementById("initialPannel");
	initPannel.parentNode.removeChild(initPannel);
}


// arguments: "next", "back", "show"
...
// the class name should be function parameter to avoid duplicates and have only one function
...
function isRandomMode() {
	return document.getElementById("rndRadioId").checked;
} // function
...
// in each iteration
function displayScore(questObj, scObj) {  // scObj

	var questNo = questObj.displayedQuestionNumber - 1;
	var userScore = scObj.get();
	var percentScore = 100 * userScore / questNo;
	var scoreText = "score: " + userScore + " / " + questNo + "<br>";
	scoreText += "[ " + percentScore.toPrecision(3) + " \%" + " ]";
...
function recordUserAnswers(questObj, ansObj) {
	...
	var i, index, answerPair = [];

	for (i = 0; i < radiosNo; i++) {
		if (radios[i].checked) {
			// record pairs of (question index: [answer1, answer2]) as (key: [value1, value2]) pairs, both as strings
			index = i + "";
			answerPair.push(index);
		}
	}
	ansObj[stringCurrIndex] = answerPair;
} // function


// delete this test function and the div "sterge" in index.html file
...
function rememberUserAnswers(questObj, ansObj) {
	...
	var chosenRadios = ansObj[questionCurrIndex];
	var chosenRadiosNo = ansObj[questionCurrIndex].length;
	
	for (i = 0; i < chosenRadiosNo; i++) {
		radios[chosenRadios[i]].checked = true;
	}
...
function computeScore(questObj, scObj) {
	...
	for (i = 0; i < radiosNo; i++) {
		choiceId = "answer" + i;
		if(document.getElementById(choiceId).checked &&	questObj.correctChoices() === i) {
		 scObj.increase();
	  }
	}
} // function
...
// in each iteration
function computeScore(questObj, scObj) { // medicine
	var radios = document.getElementsByClassName("answer");
	var radiosNo = radios.length;
	var choiceId, i, doubleCorrect=0;
	
	if (questObj.correctChoices()[1] === 9) { // question has only one answer
			for (i = 0; i < radiosNo; i++) {
				choiceId = "answer" + i;
				if(document.getElementById(choiceId).checked &&	questObj.correctChoices()[0] === i) {
				 scObj.increase();
				}
			}
	}else{ // question has two answers
			for (i = 0; i < radiosNo; i++) {
				choiceId = "answer" + i;
				if(document.getElementById(choiceId).checked &&	questObj.correctChoices()[0] === i) {
				 doubleCorrect++;
				}
				if(document.getElementById(choiceId).checked &&	questObj.correctChoices()[1] === i) {
				 doubleCorrect++;
				}
			}
			if(doubleCorrect  === 2){
